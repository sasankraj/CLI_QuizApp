...
function main() {
  ...
  const again = r.question('wanna try again? (y/any other key to quit)\t');
  if(again === 'y') {
    main();
  }
}
...
function validate(i, ch) 
{
  if(![1,2,3,4].includes(Number(ch))) {
    ...
    console.log("The correct answer is : " + questions[i]['answer'].toString());
  }
...
function conclusion(score, count, name) {
  if (score === 0) {
